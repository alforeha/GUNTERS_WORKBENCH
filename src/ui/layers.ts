...
export class LayerController {
  ...
  async generateSurfels(assetId?: string): Promise<void> {
    ...
    try {
      ...
      const streamingIndex = result.session.manifest.assets.find(
        (asset) => asset.pointCloudIndex?.sourceAssetId === target,
      )?.pointCloudIndex
      const buildInput = result.metrics.surfelIndex
      console.info(
        `[surfel-index] streaming index: ${
          streamingIndex
            ? `v${streamingIndex.indexVersion}${streamingIndex.ownership ? ` (${streamingIndex.ownership})` : ''} — untouched`
            : 'none'
        } · surfel build input: ${
          buildInput
            ? buildInput.source === 'preview'
              ? 'preview octree'
              : `${buildInput.source} v${buildInput.wpiIndexVersion} (${buildInput.built ? 'built now' : 'reused'})`
            : 'unknown'
        }`,
      )
      this.adoptSession(result.session)
